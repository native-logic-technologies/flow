#!/bin/bash
# Deploy Dream Stack with Twilio Integration and Barge-In Support
# Handles 8kHz μ-law audio from Twilio

set -e

echo "╔═══════════════════════════════════════════════════════════════════════════════╗"
echo "║         🚀 DREAM STACK - TWILIO DEPLOYMENT                                    ║"
echo "║         With Barge-In and 8kHz Audio Support                                  ║"
echo "╚═══════════════════════════════════════════════════════════════════════════════╝"
echo ""

# Configuration
export TELEPHONY_STACK_DIR="/home/phil/telephony-stack"
export MODELS_DIR="$TELEPHONY_STACK_DIR/models"
cd "$TELEPHONY_STACK_DIR"
...
pkill -f "moss_tts_fastapi" 2>/dev/null || true
pkill -f "dream_orchestrator" 2>/dev/null || true
pkill -f "hybrid_orchestrator" 2>/dev/null || true
pkill -f "twilio" 2>/dev/null || true
sleep 3
...
# ============================================================================
# STEP 1: Start MOSS-TTS (Voice) on Port 8002
# ============================================================================
echo "🎙️  Step 1: Starting MOSS-TTS (Voice) on Port 8002..."
cd "$TELEPHONY_STACK_DIR/tts"
...
if ! curl -s http://localhost:8002/health > /dev/null 2>&1; then
    echo "   ❌ MOSS-TTS failed to start"
    tail -20 /tmp/moss.log
    exit 1
fi
...
# ============================================================================
# STEP 2: Start Nemotron Brain on Port 8000
# ============================================================================
echo "🧠 Step 2: Starting Nemotron Brain on Port 8000..."
cd "$TELEPHONY_STACK_DIR"
...
nohup $VIRTUAL_ENV/bin/python -m vllm.entrypoints.openai.api_server \
    --model "$MODELS_DIR/llm/nemotron-3-nano-30b-nvfp4" \
...
BRAIN_PID=$!
echo "   PID: $BRAIN_PID"
echo "   Loading 30B model (this takes ~2 minutes)..."

# Wait for Brain with progress
for i in {1..120}; do
    if curl -s http://localhost:8000/health > /dev/null 2>&1; then
        echo ""
        echo "   ✅ Nemotron Brain ready!"
        break
    fi
    
    # Show progress
    progress=$(tail -5 /tmp/brain.log 2>/dev/null | grep -oE "[0-9]+% Completed" | tail -1)
    if [ -n "$progress" ]; then
        echo -ne "\r   Loading: $progress"
    else
        echo -ne "\r   Loading: $(tail -1 /tmp/brain.log 2>/dev/null | cut -c1-50)"
    fi
...
if ! curl -s http://localhost:8000/health > /dev/null 2>&1; then
    echo ""
    echo "   ❌ Nemotron Brain failed to start"
...
# ============================================================================
# STEP 3: Start Llama-Omni Ear on Port 8001
# ============================================================================
echo "👂 Step 3: Starting Llama-Omni Ear on Port 8001..."
cd "$TELEPHONY_STACK_DIR"
...
if ! curl -s http://localhost:8001/health > /dev/null 2>&1; then
    echo "   ❌ Llama-Omni Ear failed to start"
    tail -20 /tmp/llama_ear.log
    exit 1
fi
...
if [ "$services_ok" = false ]; then
    echo ""
    echo "❌ Some services failed to start. Exiting."
    exit 1
fi
...
# ============================================================================
# STEP 5: Start Twilio Orchestrator with Barge-In
# ============================================================================
echo "📞 Step 5: Starting Twilio Orchestrator with Barge-In on Port 8080..."
cd "$TELEPHONY_STACK_DIR"

# Create the Twilio orchestrator with barge-in if it doesn't exist
cat > twilio_dreamstack_orchestrator.py << 'ORCHESTRATOR_EOF'
#!/usr/bin/env python3
"""
...
chmod +x twilio_dreamstack_orchestrator.py
echo "✅ Twilio orchestrator created"

# Start the orchestrator
nohup $VIRTUAL_ENV/bin/python twilio_dreamstack_orchestrator.py > /tmp/orchestrator.log 2>&1 &
ORCH_PID=$!
echo "   PID: $ORCH_PID"
...
# ============================================================================
echo "╔═══════════════════════════════════════════════════════════════════════════════╗"
echo "║         ✅ DREAM STACK DEPLOYED                                               ║"
echo "╚═══════════════════════════════════════════════════════════════════════════════╝"
echo ""

# Check all services
echo "📊 Service Status:"
curl -s http://localhost:8000/health > /dev/null && echo "   ✅ Brain (Nemotron)    :8000" || echo "   ❌ Brain"
curl -s http://localhost:8001/health > /dev/null && echo "   ✅ Ear (Llama-Omni)    :8001" || echo "   ❌ Ear"
curl -s http://localhost:8002/health > /dev/null && echo "   ✅ Voice (MOSS-TTS)    :8002" || echo "   ❌ Voice"
curl -s http://localhost:8080/health > /dev/null && echo "   ✅ Twilio Orchestrator :8080" || echo "   ❌ Orchestrator"

echo ""
echo "🎯 Features Enabled:"
echo "   • Barge-in: Caller can interrupt AI speech"
echo "   • 8kHz μ-law: Twilio audio format support"
echo "   • 16kHz PCM: Ear model input format"
echo "   • 24kHz PCM: MOSS-TTS output format"
echo "   • Emotional Prosody: Sentence-level emotion tags"
echo ""
echo "📞 Next Steps:"
echo "   1. Start ngrok: ngrok http 8080"
echo "   2. Copy HTTPS URL"
echo "   3. Configure Twilio webhook: https://your-ngrok-url/twilio/inbound"
echo "   4. Call your Twilio number!"
echo ""
echo "📊 Monitor logs:"
echo "   tail -f /tmp/orchestrator.log | grep -E 'Turn|BARGE|latency|ms'"
echo ""
